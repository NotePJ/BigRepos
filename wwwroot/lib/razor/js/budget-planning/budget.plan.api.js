...
function populateDropdown(elementOrId, apiUrl, defaultText, optionCallback, showSpinner = false, initOptions = null) {
  // 🔄 ENHANCED: Support both DOM element and element ID
  let select, elementId;

  if (typeof elementOrId === 'string') {
    // Case 1: elementOrId is a string (element ID)
    elementId = elementOrId;
    select = document.getElementById(elementId);
  } else if (elementOrId && elementOrId.nodeType === Node.ELEMENT_NODE) {
    // Case 2: elementOrId is a DOM element
    select = elementOrId;
    elementId = select.id || `element_${Date.now()}`; // Generate ID if missing
    if (!select.id) select.id = elementId; // Set ID if missing
  } else {
    console.error('populateDropdown: Invalid elementOrId parameter:', elementOrId);
    return;
  }
  ...
  // Show spinner if requested and SPINNER_MAP exists
  if (showSpinner && elementId && SPINNER_MAP && SPINNER_MAP[elementId]) {
...
// - Q5: Modal + auto-close
// - Q6: Pre-check duplicate (loading message แตกต่าง)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/**
 * 💾 Save Batch Budget Entries
 *
 * @param {Array} batchData - Array of budget objects to save
 * @param {string} createdBy - Username (จาก JWT token หรือ "DevUser")
 * @returns {Promise<Object>} Response from API
 *
 * SA Answers:
 * - Q1: All or nothing - ถ้า 1 แถว error จะ rollback ทั้งหมด
 * - Q2: ไม่จำกัดจำนวนแถว
 * - Q6: Pre-check duplicate - ตรวจสอบก่อน save
 */
async function saveBatchBudgetData(batchData, createdBy = 'DevUser') {
  try {
    console.log('📤 Sending batch data to API:', {
      totalRows: batchData.length,
      createdBy: createdBy,
      endpoint: '/api/Budget/B0SaveBatchEntry'
    });

    // Q6: Loading message แตกต่างตาม batch size
    ...
    // Temporary headers for Development/Testing
    const headers = {
      'Content-Type': 'application/json'
      // JWT token will be added here when ready
    };
    ...
    console.error('❌ Error in saveBatchBudgetData:', error);
...
function getJwtToken() {
  ...
  return ''; // Temporary - return empty for now
}
...
function getCurrentUser() {
  ...
  // }
  return 'DevUser'; // Temporary for Development/Testing
}
...
// Batch Entry Save API (NEW)
window.saveBatchBudgetData = saveBatchBudgetData;
